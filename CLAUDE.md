...
| `core_vault/JobApplyFiles/06_logistics_mapping.json` | Form-ready answers (salary, notice, relocation, `qa_bank`) |
| `scripts/vault_config.py` | **Read the canonical files through this, never directly.** Redacts the private floor. |
...
- Always run `--dry-run` on the first attempt at any new portal, and after ANY change to the apply path.
- `python3 scripts/preflight_check.py` must exit 0 before a live run.
