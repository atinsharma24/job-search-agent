# Atin Sharma
**Software Engineer | Conversational AI & LLM Integration**
...
Full-stack engineer with founding experience building **production conversational AI systems** for Indian SMBs. At OpenBiz, architected **VyaparGPT** — a **WhatsApp-native LLM assistant** processing **1,000+ active users** using **Node.js, OpenAI, and Gemini APIs** with **provider-fallback logic, conversation context per session, and full message lifecycle orchestration** (webhook → LLM processing → response dispatch). Reduced API latency by **40% through PostgreSQL optimization**. Built **real-time chat infrastructure** (Socket.IO) and **AI document verification pipelines** (Gemini API). Currently building **Nimbus**, an open-source RAG document workspace on **pgvector and Groq**, enabling **semantic search and streaming LLM responses**. Expert in **Node.js, TypeScript, React, LLM API integration** (OpenAI, Gemini, Groq), **RAG pipelines, real-time systems, and conversational AI**. Targeting roles in **conversational AI and LLM-powered products**.
...
- Engineered **real-time bidirectional chat** using **Socket.IO** and integrated **VideoSDK** for live SMB-to-support workflows — supporting **low-latency conversational interfaces**
...
**Backend:** **Node.js · Express.js · NestJS · Python (Django / DRF)**  
...
✓ **Conversation Context Management & Stateful Interactions**  
